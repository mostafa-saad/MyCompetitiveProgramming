...
point toSortedPolygon(vector<point> &p) { // Sort simple polygon
	int mi = 0;
	rep(i, p) if( make_pair(p[i].Y, p[i].X) < make_pair(p[mi].Y, p[mi].X))
		mi = i;
	
	point p0 = p[mi];
	swap(p[mi], p.back());		p.pop_back();
	 
	sort( all(p), pcmp(p0));
	return p0;
}

int linePointsUp(vector<point> points)
{
	if( sz(points) < 3)		return sz(points);
	int mx = 2;
	
	while( sz(points) > 2) {
		point p0 = toSortedPolygon(points);
		int m = 2;
		lp(i, sz(points)-1){
			if( cp(points[i]-p0, points[i+1]-p0) ==0) 	m++;
