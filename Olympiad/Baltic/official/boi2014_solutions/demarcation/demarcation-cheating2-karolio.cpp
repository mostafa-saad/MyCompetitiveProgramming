...
int nextHorizontal(ii p[], int len, int pnt, ll &toinc, ll &todec, int &toince, int &todece)
{
	do {
		toinc += edgeLen(pnt); toince++;
		pnt = (pnt + 1) % len;
		todec -= edgeLen(pnt); todece--;
	} while (p[pnt].second != p[(pnt + 1) % len].second);
	return pnt;
}

int prevHorizontal(ii p[], int len, int pnt, ll &toinc, ll &todec, int &toince, int &todece)
{
	do {
		todec += edgeLen(pnt); todece++;
		pnt = (pnt - 1 + len) % len;
...
{
	int begin = nextHorizontal(p, len, 0, B, A, Be, Ae);
	E = F = begin;
	A = -edgeLen(E); B = P - edgeLen(E);
	int Ae = -1, Be = len - 1;
	do {
		while (edgeLen(E) + edgeLen(F) >= A - B) {
			if (edgeLen(E) + edgeLen(F) >= abs(A - B) && abs(Ae - Be) <= 4) {
			...
			F = nextHorizontal(p, len, F, A, B, Ae, Be);
		}
		F = prevHorizontal(p, len, F, A, B, Ae, Be);
		E = nextHorizontal(p, len, E, B, A, Be, Ae);
	} while (E != begin);
	return false;
}
