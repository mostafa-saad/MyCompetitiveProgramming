...
bool getVerticalSplit(ii p[], int len, int &x, int &y1, int &y2)
{
	...
	for (int E = 0; E < len; E++) if (p[E].second == p[(E + 1) % len].second)
		for (int F = E + 1; F < len; F++) if (p[F].second == p[(F + 1) % len].second) {
			A = 0;
			for (int M = E + 1; M < F; M++) A += edgeLen(M);
			B = P - edgeLen(E) - edgeLen(F) - A;
