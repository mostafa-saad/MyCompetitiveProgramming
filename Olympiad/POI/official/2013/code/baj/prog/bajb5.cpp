 ...
 *                         Zamiast sprawdzac wszystkie maksymalne bloki  *
 *                         zerowe, sprawdza tylko najdluzsze z nich.     *
 *                                                                       *
 *************************************************************************/
...
int main(void) {
    ...
    if (c[0] == 0) {
        ...
        res = min(res, best);
        // jesli na koncu nie ma bloku zerowego
        for (int i = 0; i < n; ++i) {
            if (c[i] == 1) {
                int can = 2 * get(0, i - 1, 1) + get(0, i - 1, 0) +
                          2 * get(i, n - 1, -1) + get(i, n - 1, 0);
                res = min(res, can);
            }
        }
    }
