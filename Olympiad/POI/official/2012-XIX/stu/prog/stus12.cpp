...
struct kr {
  ...
  int d;
  kr() {}
  kr(int nr, int d) : nr(nr), d(d) {}
  bool operator>(const kr& rhs) const {
    return d > rhs.d;
  }
};

priority_queue<kr, vector<kr>, greater<kr> > Q;
...
long long wyrownaj(int p) {
  ...
  while(!Q.empty())
  {
    int i = Q.top().nr;
    Q.pop();
    if(i > 1 && pom[i] + p < pom[i-1]) {
      ruchy += pom[i-1] - (pom[i] + p);
      pom[i-1] = pom[i] + p;
      Q.push(kr(i-1, pom[i-1]));
    }
    if(i < n && pom[i] + p < pom[i+1]) {
      ruchy += pom[i+1] - (pom[i] + p);
      pom[i+1] = pom[i] + p;
      Q.push(kr(i+1, pom[i+1]));
    }
