...
var
  n,k,a,b,i,j,dlg1,dlg2,cyklInd,wyn1,wyn2,topo_ind : longint;
  graf, symbol, dlg, nr, gleb, topo : array[0 .. MAXN] of longint;
...
begin
  ...
  for i := 0 to MAXN do begin
    graf[i] := 0; symbol[i] := 0; dlg[i] := 0; 
    nr[i] := 0; gleb[i] := 0; topo[i] := 0;
    vis[i] := false;
    kraw[i] := nil;
  end;
  ...
  // wyznaczenie glebokosci dla wierzcholkow
  ...
  for i := 1 to k do begin
    ...
    if(symbol[a] = symbol[b]) and (symbol[a] > 0) then begin
      while gleb[a] > gleb[b] do begin a := graf[a]; inc(wyn1); end;
      while gleb[a] < gleb[b] do begin b := graf[b]; inc(wyn2); end;
      while a <> b do begin a := graf[a]; b := graf[b]; inc(wyn1); inc(wyn2); end;
    end;
    ...
    if(symbol[a] = symbol[b]) and ((symbol[a] < 0) and (a <> b)) then begin
      ...
      if nr[a] > nr[b] then swap(dlg1, dlg2);
