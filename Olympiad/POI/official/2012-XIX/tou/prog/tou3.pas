...
var
  ...
  rozmiar : array[0..MAXN] of longint; (* rozmiary zbiorow rozlacznych *)
...
begin
  a := find(a); b := find(b);
  if a <> b then
    if rozmiar[a] > rozmiar[b] then
    begin
      ojciec[b] := a;
      rozmiar[a] := rozmiar[a] + rozmiar[b];
    end else begin
      ojciec[a] := b;
      rozmiar[b] := rozmiar[b] + rozmiar[a];
    end;
end;
...
begin
  readln (n, m, k);
  for i := 1 to n do begin ojciec[i] := i; rozmiar[i] := 1; end;
  for i := 1 to m do
  begin
    readln (a, b);
    if a > k then
