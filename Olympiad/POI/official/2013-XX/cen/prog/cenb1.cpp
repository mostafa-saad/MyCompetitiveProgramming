 ...
 *   Opis:                 Rozwiazanie bledne, rozpatruje graf E^2       *
 *                         a nie E^2\E                                   *
 *                                                                       *
 *************************************************************************/
...
struct State {
    ...
    int dist;
    State(bool _even, int _current, int _dist):even(_even), current(_current), dist(_dist) {}
    ...
    while (!R.empty()) {
        ...
        if (cur.even) {
            ...
            for (int i = 0; i < (int)edges2[cur.current].size(); ++i) {
                // tutaj jest blad - nie sprawdza sie czy mozemy przejsc ta krawedzia
                if (dist2[edges2[cur.current][i]] == INF) {
                    dist2[edges2[cur.current][i]] = cur.dist + 1;
                    R.push(State(true, edges2[cur.current][i], cur.dist + 1));
                }
                // nie usuwamy tez krawedzi
