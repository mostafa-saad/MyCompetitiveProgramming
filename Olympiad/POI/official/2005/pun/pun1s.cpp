 ...
 *   Opis:                Rozwi±zanie nieoptymalne 1                     *
 *                                                                       *
 *************************************************************************/
...
ld equals(ld v1, ld v2, int &diffDigit)
{
  ...
  ld diff = 1.0L;
  diffDigit = 0;
  while (diffDigit < 32 && diff > result) {
    diff /= 10;
    ++diffDigit;
  }
...
void sortByAngles(vector < Point > &set, vector < Point > &result)
{
  ...
  sort(result.begin(), result.end(), ltstr());
}
...
bool zero(Point & p)
{
  return equals(p.first, 0) && equals(p.second, 0);
}
