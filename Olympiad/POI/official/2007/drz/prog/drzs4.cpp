...
int zysk(int d1, int d2) {
  ...
  if (bez(d1,d2)==1) n_koszt+=2*bez(wys[d1],wys[d2]);
...
void oblicz(int drzewo) {
  ...
  if (drzewo>1) {tmp=zysk(drzewo, drzewo-1); if (tmp>akt_zysk) akt_zysk=tmp;}
  if (drzewo<n) {tmp=zysk(drzewo, drzewo+1); if (tmp>akt_zysk) akt_zysk=tmp;}
  
  for (int i=0; i<n; i++) {
    if (mozl_zysk[drzewo]+sort[i].kwota<=akt_zysk) break;
    if (bez(sort[i].nr_drzewa,drzewo)>1) {
