 ...
 *                        kata miedzy nimi. Oszczedna, tzn. odrzuca      *
 *                        nieprzydatne punkty z sortowania               *
 *                                                                       *
 *************************************************************************/
 ...
 * porzadek sortowan jest taki, by dla a < ile_pop i ile_pop <= c1 < c2 < ile_pop+ile_nast zachodzilo: w_prawo (a,b,c1) => w_prawo(a,b,c2) */
  for (int b = 1; b < n; ++b) {
    for (int a = (b == 1 ? 0 : 1); a < b; ++a)  //tylko do oazy nr 1 mozna pojsc z oazy nr 0 !!!
      if (VEC(a, b) <= VEC(a, 0))
        oaz[b].sas[oaz[b].ile_pop++] = a; //odrzucamy te oaz, z ktorych nie da sie dojsc nie przecinajac naszej trasy
    for (int c = b + 1; c < n; ++c)
      if (VEC(b, c) <= VEC(b, 0))
        oaz[b].sas[oaz[b].ile_pop + oaz[b].ile_nast++] = c; //odrzucamy te oaz, do ktorych nie da sie dojsc nie przecinajac naszej trasy
