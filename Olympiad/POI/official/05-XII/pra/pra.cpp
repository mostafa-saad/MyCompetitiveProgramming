...
inline int MAX(int a, int b)
{
  return (a > b ? a : b);
                    ...
                    // - tutaj w linii 113. to by nie wyszlo, bo dwukrotnie wykonalibysmy a++
...
{
  if (x >= 0)
    return (y > 0 ? (Duza * x) / (x + y) : 2 * Duza - (Duza * x) / (x - y));
  else
    return (y < 0 ? 2 * Duza + (Duza * x) / (x + y) : 4 * Duza -
             (Duza * x) / (x - y));
}
 ...
 * porzadek sortowan jest taki, by dla a < ile_pop i ile_pop <= c1 < c2 < ile_pop+ile_nast zachodzilo: w_prawo (a,b,c1) => w_prawo(a,b,c2) */
  for (int b = 1; b < n; ++b) {
    for (int a = (b == 1 ? 0 : 1); a < b; ++a)  //tylko do oazy nr 1 mozna pojsc z oazy nr 0 !!!
      if (VEC(a, b) <= VEC(a, 0))
        oaz[b].sas[oaz[b].ile_pop++] = a; //odrzucamy te oazy, z ktorych nie da sie dojsc nie przecinajac naszej trasy
    for (int c = b + 1; c < n; ++c)
      if (VEC(b, c) <= VEC(b, 0))
        oaz[b].sas[oaz[b].ile_pop + oaz[b].ile_nast++] = c; //odrzucamy te oazy, do ktorych nie da sie dojsc nie przecinajac naszej trasy

    sortWzgledemPoKacie(b, VEC(b, 0), oaz[b].sas, oaz[b].ile_pop);
    sortWzgledemPoKacie(b, VEC(0, b), oaz[b].sas + oaz[b].ile_pop,
                        oaz[b].ile_nast);
  }
}
...
/* Sprawdza, czy zakret pun[a],pun[b],pun[c] jest zakretem w prawo */
  return (VEC(a, b) <= VEC(b, c));  //to wystarczy tylko dlatego, ze odrzucilismy juz tych poprzednikow i nastepcow, z ktorych nie da sie dojsc bez przecinania trasy
}
