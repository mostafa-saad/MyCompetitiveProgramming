...
int graf[MAXN], symbol[MAXN], l[MAXN][MAXP], gleb[MAXN];
int n, k, a, b, c, i, dlg1, dlg2, temp, cyklInd, wyn1, wyn2;
bool vis[MAXN];
VI topo, kraw[MAXN];
...
int main(){
  ...
  FOR(i,1,k) {
    ...
    if(symbol[a] == symbol[b] && symbol[a] < 0 && a != b){
      temp = b;
      while(temp != a) { temp = graf[temp]; dlg2++; }
      temp = a;
      while(temp != b) { temp = graf[temp]; dlg1++; }
