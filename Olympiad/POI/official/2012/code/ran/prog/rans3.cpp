...
int graf[MAXN], symbol[MAXN], dlg[MAXN], nr[MAXN],  gleb[MAXN];
...
int main(){
  ...
  FOR(i,1,k) {
    ...
    if(symbol[a] == symbol[b] && symbol[a] > 0){
      while(gleb[a] > gleb[b]) {a = graf[a]; wyn1++;}
      while(gleb[a] < gleb[b]) {b = graf[b]; wyn2++;}
      while(a != b) { a = graf[a]; b = graf[b]; wyn1++; wyn2++;}
    }
    
    // spotkanie niemozliwe
    if(symbol[abs(symbol[a])] != symbol[abs(symbol[b])]) wyn1 = wyn2 = -1;
