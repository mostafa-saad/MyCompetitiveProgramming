...
/* Rozwiazanie weryfikujace do zadania SZA
 * Autor: Miroslaw Michalski
 * 25NOV11
 */
#include <cstdio>
...
int main() {
  ...
  for(int i = 0; i < n; i++) {
    ...
    v.push_back(std::make_pair(std::make_pair(2 * b, c), std::make_pair(a, -1)));
    totWeight += a;
  }
  assert(scanf("%d", &m));
  for(int i = 0; i < m; i++) {
    assert(scanf("%d%d%d", &a, &b, &c));
    v.push_back(std::make_pair(std::make_pair(2 * a + 1, a + c), std::make_pair(b, i)));
