 ...
 *                        przechowujac w wierzcholku stan klocka         *
 *                        i maksimum potomkow, ale odcinajac             *
 *                        przeszukiwanie - nie tnie, gdy prostokat       *
 *                        zapelnia cala szerokosc oraz jesli prostokat   *
 *                        sie zawiera w calosci w jednej czesci, schodzi *
 *                        tylko do niej.                                 *
 *                                                                       *
 *************************************************************************/
...
int jak_wysoko(int a,int b,int c,int d,int i,int j,int p,int q,int r,int s){
  ...
  /* Dzielimy wzdluz x, jesli ma to sens - czyli szerokosc >1, prostokat nie zajmuje calej szerokosci 
     i nie jest w calosci w jednej z polowek przy podziale wzdluz y*/
  if (i < 1024 && (!(a <= p && c >= r)) && (!(b >= (q+s)/2 || d <= (q+s)/2))){
  ...
  /* Dzielimy wzdluz y, jesli ma to sens - czyli szerokosc >1, prostokat nie zajmuje calej szerokosci
     i nie jest w calosci w jednej z polowek przy podziale wzdluz x */
  if (j < 1024 && (!(b <= q && d >= s)) && (!(a >= (p+r)/2 || c <= (p+r)/2))){
