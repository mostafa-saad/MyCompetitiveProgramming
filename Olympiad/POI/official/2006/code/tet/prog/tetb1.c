...
int jak_wysoko(int a,int b,int c,int d,int i,int j,int p,int q,int r,int s){
  ...
  /* Dzielimy wzdluz x, jesli ma to sens - czyli szerokosc >1 i prostokat nie zajmuje calej szerokosci */
  if (i < 1024 && (!(a <= p && c >= r))){
  ...
  /* Dzielimy wzdluz y, jesli ma to sens - czyli szerokosc >1 i prostokat nie zajmuje calej szerokosci */
  if (j < 1024 && (!(b <= q && d >= s))){
