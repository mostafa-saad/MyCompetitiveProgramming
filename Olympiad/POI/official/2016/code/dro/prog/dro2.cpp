 ...
 *   Opis:                 Rozwiazanie wzorcowe oparte na przedzialach        *
 *****************************************************************************/
...
namespace GV {
  ...
  int the_shortest_edge_is_from[MaxN + 5];

  std::vector< std::pair<int, int> > MakeBadRanges() {
    for (int i = 1; i <= n; i++) {
      the_shortest_edge_is_from[i] = 0;
    }
    for (int from = 1; from <= n; from++) {
      for (const int j : graph[topological_order[from]]) {
        int to = topological_order_t[j];
        assert(from < to);
        the_shortest_edge_is_from[to] = from;
      }
    }
    std::vector< std::pair<int, int> > result;
    for (int i = 1; i <= n; i++) {
      if (the_shortest_edge_is_from[i] + 1 <= i - 1) {
        result.push_back({the_shortest_edge_is_from[i] + 1, i - 1});
      }
    }
    return result;
  }
  ...
  std::vector<int> OneWayGoodVertices() {
    std::vector< std::pair<int, int> > bad_ranges = MakeBadRanges();
    std::vector<int> bad_positions = B::SumOfRanges(bad_ranges, n);
    std::vector<int> good_positions = B::Complement(bad_positions, n);
    std::vector<int> good_vertices;
    for (int i : good_positions) {
      good_vertices.push_back(topological_order[i]);
    }
