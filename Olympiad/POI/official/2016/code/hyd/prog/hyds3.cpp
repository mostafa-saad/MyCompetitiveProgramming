 ...
 *                         Sknocone jest liczenie odleglosci do najblizszych  *
 *                         wygrywajacych pekow                                *
 *****************************************************************************/
...
void dfs_to_up(int cyc_v, bool is_overcact_win) {
  ...
  for (int i = 1; i < cyc_len; i++) {
    ...
    for (int dir = -1; dir <= 1; dir += 2) {
      if (dir == -1) {
        ...
        start_ind = (i + 1) % cyc_len;
      }
      int dis = 0;
      dis_to_win[act_v][(1 - dir) / 2] = -1;
      for (int j = start_ind; j != i; j = (j + dir + cyc_len) % cyc_len) {
        dis++;
        if ((j == 0 && is_overcact_win) || (j != 0 && win_down[cyc[cyc_v][j]])) {
          dis_to_win[act_v][(1 - dir) / 2] = dis;
          break;
        }
      }
    }
  ...
  for (size_t i = 1; i < cyc[cyc_v].size(); i++) {
    ...
    for (int cyc_nei : cyc_from_v[act_v]) {
      ...
      dfs_to_up(cyc_nei, cnt_cact_all_from_v[act_v][tA] || cnt_cact_all_from_v[act_v][t1] % 2);
      cnt_cact_all_from_v[act_v][child_type]++;
    } 
  }
}
