...
/* Algorytm binarny liczenia NWD. */
Natural nwd(Natural x,Natural y)
{
  Natural z(1);
  if (x==Natural(0)) return y;
  if (y==Natural(0)) return x;
  while (x!=y)
  {
    if (!(x.digs[0]%2) && !(y.digs[0]%2))
    {
      z=z*2;
      x=x/2; y=y/2;
    }
    else if (!(x.digs[0]%2)) x=x/2;
    else if (!(y.digs[0]%2)) y=y/2;
    else if (y>x) y=y-x;
    else x=x-y;
  }
  return z*x;
...
int main()
{
  ...
  FORD(i,n-2,0)
  {
    ...
    Natural d=nwd(licznik,mianownik);
    licznik/=d; mianownik/=d;
    liczniki[i]=licznik; mianowniki[i]=mianownik;
    if (liczniki[i]==Natural(0))
    {
      /* Optymalizacja: dojscie do 0/1. */
      REP(j,i)
      {
        liczniki[j]=liczniki[i];
        mianowniki[j]=mianowniki[i];
      }
      break;
    }
  }
