 ...
 *   Opis:     Rozwiazanie bledne, o zlozonosci czasowej O(n); blednie   *
 *             wyznacza wynik dla maksymalizacji liczby ofiar.           *
 *                                                                       *
 *************************************************************************/
...
int licz(int v, int jak, int root)
{
  ...
  else /* nie bierzemy v do zbioru */
  {
    ...
    for (int i = 0; i < (int)t[v].size(); i++)
      if (t[v][i] != root)
      {
        int x = max(licz(t[v][i], 1, root), licz(t[v][i], 2, root));
        if (x == -INFTY)
          return w = -INFTY;
        w += x;
      }
...
/* Fragment zawierajacy rozwiazanie bledne */
vector<int> kolejnosc;
bool zywy[MAX_N];

void postorder(int v, int root)
{
  for (int i = 0; i < (int)t[v].size(); i++)
    if (t[v][i] != root)
      postorder(t[v][i], root);
  kolejnosc.push_back(v);
}

void bledne_maxofiar()
{
  for (int i = 0; i < (int)roots.size(); i++)
    postorder(roots[i], roots[i]);
  reverse(kolejnosc.begin(), kolejnosc.end());
  for (int i = 0; i < n; i++)
    zywy[i] = true;
  for (int i = 0; i < (int)kolejnosc.size(); i++)
    if (zywy[kolejnosc[i]])
      zywy[s[kolejnosc[i]]] = false;
  max_ofiar = 0;
  for (int i = 0; i < n; i++)
    if (zywy[i])
      max_ofiar++;
