...
long long wekt(point p1, point p2) {
  long long tmp = ((long long)p1.x) * ((long long)p2.y) - ((long long)p1.y) * ((long long)p2.x);
  if (tmp>=0) return tmp;
  return (-1)*tmp;
}

//oblicza 2 * (pole trojkatow o najmniejszym wierzcholku nr)
long long pole(int nr) {
  long long wyniktmp; //wynik tymczasowy
...
int main() {
  long long wynik2; //wynik zwiekszony dwukrotnie
