 ...
 *   Opis:     Rozwiazanie bledne, ktore nie uwzglednia remisow          *
...
int main()
{
    ...
    for(int j=0;j<p;j++)
    {
        ...
        else if(abs(a-c)==abs(b-d))
        {
            if(a+b==c+d)
            {
                x3.push_back( wekt( inf+c-b-1,  3*j+komu,  1, 0, inf ) );
                x3.push_back( wekt( inf+c-b,    3*j+2,     1, 0, inf ) );
            }
            else
            {
                x4.push_back( wekt( b+c-1,      3*j+komu,  1, 0, inf  ) );
                x4.push_back( wekt( b+c,        3*j+2,     1, 0, inf  ) );
            }
        }
        else if(abs(a-c)>abs(b-d))
        {
