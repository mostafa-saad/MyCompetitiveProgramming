 ...
 *             Szukamy skojarzenia, ale w zlym grafie.                   *
 *                                                                       *
 *************************************************************************/
...
bool findPath(int s) {
    ...
    for(size_t i=0; i < flow[s].size(); i++)
        if (info[flow[s][i].second].first>0 && !visited[flow[s][i].first] && findPath(flow[s][i].first)) {
            info[flow[s][i].second].first--;
            info[info[flow[s][i].second].second].first++;
            return true;
        }
    return false;
}
...
bool getFlow(int value, int goFurther=0) {
    ...
    for(size_t i=0; i<t.size(); i++) {
        ...
        if (p <= value && q<=value) {
            addEdge(a+1, n+b+1, 1, i, 0); 
            addEdge(b+1, n+a+1, 1, i, 1);
        } else 
            if (p <= value) {
                addEdge(a+1, n+b+1, 1, i, 0);
            } else
    ...
    if (!goFurther) 
        return (fl==m);
    for(int i=0; i<edges-2*(n+n); i++) if (!info[i].first && core[i]) {
