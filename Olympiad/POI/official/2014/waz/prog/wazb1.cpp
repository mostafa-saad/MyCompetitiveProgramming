 ...
 *                         Takie jak wzorcowe, lecz ma nieostroznie      *
 *                         dobrane typy danych.                          *
 *                         Potrzebuje >600 MB pamieci i jest powolne.    *
 *                                                                       *
 *************************************************************************/
...
int hamilton2_t[2][M][3][3*M+1][3];
int hamilton2_wypisz[2][M][3][3*M+1][3]; // uzywane tylko jesli y1,y2!=1 (inaczej nie ma wyboru)
const int RUSZ_Y1=0;
const int RUSZ_Y2=1;
...
int hamilton2(int kier,int x,int y1,int val1,int y2)
{
    if (x==n && abs(y1-y2)==1) return true; // there was a BUG
    if (x==n) return false;
    int &w=hamilton2_t[kier][x][y1][val1][y2];
    if (!ustaw && w!=-1) return w;
    int &wo=hamilton2_wypisz[kier][x][y1][val1][y2];
...
int hamilton(int kier,int x,int y,int monot)
{
    if (x==n) return true;
    int &w=hamilton_t[kier][x][y][monot];
    if (!ustaw && w!=-1) return w;
