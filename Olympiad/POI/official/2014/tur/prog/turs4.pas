...
begin
	...
	begin
		...
		begin
			...
			if (X and nei) > 0 then
				cover := one;
			if next[X][Y or cover] > act[X][Y] then
				next[X][Y or cover] := act[X][Y];
			if next[X or one][Y or one or nei] > act[X][Y] + c[v] then
				next[X or one][Y or one or nei] := act[X][Y] + c[v];
		end;
	end else begin
		where[v] := -1;
		i := head[v];
		...
		begin
			...
			Y := validY[t][i];
			if (Y and one) > 0 then
				if next[X and (not one)][Y and (not one)] > act[X][Y] then
					next[X and (not one)][Y and (not one)] := act[X][Y];
		end;
	end;
	t := newt;
	for i := 1 to validC[t] do
