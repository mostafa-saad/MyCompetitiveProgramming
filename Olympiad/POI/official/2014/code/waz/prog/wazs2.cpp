...
int zygzak(int kier,int x1,int y,int x2,int monot)
{
    ...
    FORD(i,x2,x1) {
        if (!zgodne(t[kier][i][1],akt)) return false;
        akt+=diff;
    }
    FOR(i,x1,x2) {
        if (!zgodne(t[kier][i][2-y],akt)) return false;
        akt+=diff;
    }
    return true;
}
...
int zawijas(int kier,int x1,int y1,int len,int y2,int monot)
{
    ...
    int x2=x1+len-1;
    FOR(i,x1,x2) {
        if (!zgodne(t[kier][i][y1],akt)) return false;
        akt+=diff;
    }
    ...
    diff=-diff;
    FORD(i,x2,x1) {
        if (!zgodne(t[kier][i][3-y1-y2],akt)) return false;
        akt+=diff;
    }
    FOR(i,x1,x2) {
        if (!zgodne(t[kier][i][y2],akt)) return false;
        akt+=diff;
    }
    return true;
}
...
char hamilton2_t[2][M][3][3*M][3];
char hamilton2_wypisz[2][M][3][3*M][3]; // uzywane tylko jesli y1,y2!=1 (inaczej nie ma wyboru)
const int RUSZ_Y1=0;
const int RUSZ_Y2=1;
...
short hamilton(int kier,int x,int y,int monot)
{
    if (x==n) return true;
    short &w=hamilton_t[kier][x][y][monot];
    if (!ustaw && w!=-1) return w;
