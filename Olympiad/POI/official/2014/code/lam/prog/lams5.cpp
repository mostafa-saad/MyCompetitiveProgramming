...
struct node {
    sptr *next;
    int key, h;
    node (int k = -1, int _h = 0) : next(NULL), key(k), h(_h) {
        next = new sptr[h];
    }
};
...
struct skiplist {
    ...
    void extend(int hh) {
        if (hh > head.h) {
            int nh = hh;//min(max(hh, 2 * head.h), MAXH);
            sptr* tmp = new sptr[nh];
            REP (i, head.h) {
                tmp[i] = head.next[i];
            }
            int sz = size();
            for (int i = head.h; i < nh; ++i) {
                tmp[i].len = sz;
            }
            delete head.next;
            head.next = tmp;
    ...
    void insert(int k) {
        ...
        for (int q = H; q < head.h; ++q) {
            ++vis_st[q].p->next[q].len;
        }
    }
    int size() {
        node *l = &head;
        int res = adv(l, head.h - 1, INF);
        return res + l->next[head.h - 1].len;
    }
};
