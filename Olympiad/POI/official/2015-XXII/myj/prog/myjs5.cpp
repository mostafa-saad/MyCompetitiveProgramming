...
int ComputeRange(int from, int to, int priceIdx) {
    int res = 0;
    for (int i = nPeople-1; i >= priceIdx; i--)
        if ((people[i].from >= from) && (people[i].to <= to))
            res++;
    return res;
}
...
int Compute(int from, int to, int priceIdx) {
    ...
    for (int pos = from; pos <= to; pos++) {
        ...
        int peopleCount = ComputeRange(from, to, priceIdx);
        if (pos-1 >= from) peopleCount -= ComputeRange(from, pos-1, priceIdx);
        if (pos+1 <= to) peopleCount -= ComputeRange(pos+1, to, priceIdx);
