 ...
 *   Opis:     Rozwiazanie prawie wzorcowe O((n+m)*log(n)) w ktorym      *
 *             brakuje long longow                                       *
 *                                                                       *
 *************************************************************************/
...
inline void ustaw(int x, int w)
{
	...
	int v = ile + x;
	drz[v].suma += w;
	drz[v].mx_lewy = drz[v].mx_prawy = drz[v].mx = max(drz[v].suma, 0);
