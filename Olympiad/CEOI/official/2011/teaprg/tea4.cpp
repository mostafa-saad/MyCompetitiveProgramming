 ...
 * Task: Teams
 *  Model solution 5
 *  Same as model solution 3, but the output differs in some white spaces
 * Author: Jakub Lacki
 */
...
int main()
{
    ...
    previous[0] = 0;
    int res = check(n, n);
    int jump = 1, cur_team = n;
    while(2*jump <= n) jump *= 2;
    while(jump)
    {
        if(cur_team - jump >= 1 && check(n, cur_team-jump) == res)
            cur_team -= jump;
        jump /= 2;
    }
    check(n, cur_team);

    printf("%d", teams_count[n]);
    vector<int> team;
    int cur = n;
    while(cur > 0)
    {
